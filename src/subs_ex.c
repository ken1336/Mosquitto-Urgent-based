...
static int subs__process(struct mosquitto_db *db, struct mosquitto__subhier *hier, const char *source_id, const char *topic, int qos, int retain, struct mosquitto_msg_store *stored, bool set_retain)
{
	...
	while(source_id && leaf){
		...
		if(rc2 == MOSQ_ERR_ACL_DENIED){
			...
			if(db__message_insert(db, leaf->context, mid, mosq_md_out, msg_qos, client_retain, stored) == 1){
...
int sub__messages_queue(struct mosquitto_db *db, const char *source_id, const char *topic, int qos, int retain, struct mosquitto_msg_store **stored)
{
	...
	assert(db);
	assert(topic);
	//printf("message queue: %s	qos: %d\n",source_id,qos);
	/*if((*stored)->payload.ptr!=NULL){
		const void *payload;
		payload = UHPA_ACCESS_PAYLOAD(*stored);
		
		printf("len:%d,%s\n",(*stored)->payloadlen,payload);
...
static int subs__process_ex(struct mosquitto_db *db, struct mosquitto__subhier *hier, const char *source_id, const char *topic, int qos, int retain, struct mosquitto_msg_store *stored, bool set_retain,Subscriptionlist **subs)
{
	int rc = 0;
	int rc2;
	int client_qos, msg_qos;
...
#endif
		if(hier->retained){
			db__msg_store_deref_ex(db, &hier->retained);
#ifdef WITH_SYS_TREE
			db->retained_count--;
#endif
		}
	...
	while(source_id && leaf){
		...
		if(rc2 == MOSQ_ERR_ACL_DENIED){
			...
			struct mosquitto_client_msg* umsg = NULL;
			if(db__message_insert_ex(db, leaf->context, mid, mosq_md_out, msg_qos, client_retain, stored,&umsg) == 1){
				
 				rc = 1;
			}
			printf("umsg: %s\n",umsg->store->topic);
			add_sub(subs,leaf->context,umsg);
			rc = 1;
			//display_subs(subs);
			//printf("sub__process!!!!!!!!!!: %s\n", leaf->context->id);
		}else{
			return 1; /* Application error */
		}
		leaf = leaf->next;
...
static void sub__search_ex(struct mosquitto_db *db, struct mosquitto__subhier *subhier, struct sub__token *tokens, const char *source_id, const char *topic, int qos, int retain, struct mosquitto_msg_store *stored, bool set_retain,Subscriptionlist **subs)
{
	/* FIXME - need to take into account source_id if the client is a bridge */
	struct mosquitto__subhier *branch, *branch_tmp;
	bool sr;
	...
	HASH_ITER(hh, subhier->children, branch, branch_tmp){
		...
		if(tokens && UHPA_ACCESS_TOPIC(tokens)
			...
			sub__search_ex(db, branch, tokens->next, source_id, topic, qos, retain, stored, sr,subs);
			if(!tokens->next){
				subs__process_ex(db, branch, source_id, topic, qos, retain, stored, sr,subs);
			 ...
			 * there may still be other subscriptions to deal with.
			 */
			subs__process_ex(db, branch, source_id, topic, qos, retain, stored, false,subs);
...
int sub__messages_queue_ex(struct mosquitto_db *db, const char *source_id, const char *topic, int qos, int retain, struct mosquitto_msg_store **stored,Subscriptionlist **subs)
{
	...
	*/
	...
	if(subhier){
		...
		sub__search_ex(db, subhier, tokens, source_id, topic, qos, retain, *stored, true,subs);
	}
	sub__topic_tokens_free(tokens);

	/* Remove our reference and free if needed. */
	db__msg_store_deref_ex(db, stored);
